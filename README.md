...
| Firebase Firestore | 排行榜 `scores`（公开读/写，**仅站长可删**）+ `special`（呼吸灯名单，公开读、仅站长写） |
...
│   ├── admin.html        # 站长管理页（增删改分数 / 删某人全部 / 呼吸灯名单）
...
- **站长后台**：排行榜管理页 / 玩家汇总 / 呼吸灯授权名单，仅站长账号（登录身份经 Firebase 规则 `isOwner` 校验）可用；删除与名单写入均走 `isOwner` 校验，其他人无法操作。
