...
🔗 线上主站：https://maomao1ovo1.github.io （短域名 `https://maomaowang.top` 已设为自动跳转到主站）
...
- 👤 **登录系统（Firebase）**：Google / GitHub / 邮箱 / 游客四门可用；支持绑定/解绑三方账号、**邮箱+密码登录需先在邮箱点「确认」链接验证通过**（注册即发验证邮件，未验证会被拦截登录）、**更改密码**（旧密码验证 + 两次确认的常规软件体验）。
- 🎵 **音乐页 & 相册页**：私藏的小角落。
