...
var BUILD_CODE = 109;                    /* v9.19：接续 v9.17 的两项改动（画布 touch-action + 删死字段）；
                                           编号跳过 108 —— 毛毛手机上装的是 v9.18（玻璃质感那版），
                                           版本号必须更高才能覆盖安装；与 version.json 同步 */
...
function tutBarText(){
  /* v10.0：任务条退役，这里不再产出任何「任务 N/M」文案（保留空函数以免旧调用点报错）*/
...
function tutBarSync(){
  var el = document.getElementById('tutBar');
  if (!el) return;
  /* v10.0 修 bug（毛毛截图：教学关里还挂着「任务 1/0：undefined」）：
     任务清单已撤（TUT_TOTAL=0、TUT_STEPS=[]），但这里还在按 tutorial 显示任务条，
     于是拼出 1/0 与 undefined。现改为**永远隐藏**，任务条正式退役。 */
...
/* ══════════════════════════════════════════════════════════════════════════
 * v9.12 统一开局状态清理（毛毛：「不希望有 bug」→ 系统性 debug 的产物）
 *
 * 背景：原来只在 startLevel 里零散重置了十几个变量，而**跨局状态远不止这些** ——
 *   漏掉的那批会从上一局漏进来。实际会出这些症状：
 *     · skillMode 残留   → 新局一进去就在等「点一个目标放技能」
 *     · banMode / 手牌残留 → 新局自带上一局的禁卡表与换批次数
 *     · buffChoiceOpen   → 选卡面板状态错乱
 *     · menuPause        → 新局一进就被当成「菜单打开中」而暂停
 *     · sideOpen         → 左侧信息栏自动展开
 *     · pvOn / pvTower   → 战场上残留一圈射程预览
 *     · comboT / comboTxt → 第 0 波就飘出上一局的连杀提示
 * 做法：把「一局之内必须是干净初值」的状态集中到这里，startLevel 一进来先调它。
 *    以后新增任何局内状态，都加到这个函数里，别再零散地写（这就是上次漏的原因）。
 * ══════════════════════════════════════════════════════════════════════════ */
function resetRunState(){
  /* —— 主动技能：取消「等待选目标」状态 —— */
  skillMode = null;
  if (typeof skillBtns !== 'undefined') skillBtns = [];
  /* —— 强化卡：关闭选卡面板 + 重置换批/禁卡工具与手牌 —— */
  buffChoiceOpen = false;
  if (typeof closeBuffPanel === 'function'){ try { closeBuffPanel(); } catch (e) {} }
  if (typeof resetDraftTools === 'function'){ try { resetDraftTools(); } catch (e) {} }
  /* —— 面板 / 暂停 —— */
  menuPause = false;
  lastPanel = null;
  /* 注意：sideOpen（左侧信息栏展开/折叠）**故意不重置** ——
     它属于「用户偏好」（和文字详略、音效开关同类），不是局内状态。
     v9.12 曾把它当跨局状态清掉，等于每局强行折叠用户的侧栏（deep40 测试因此挂掉）。
     约定：resetRunState 只清「一局之内必须是初值」的东西，用户偏好一律保留。 */
  /* —— 射程预览 —— */
  pvOn = false; pvC = -1; pvR = -1; pvElem = ''; pvTower = null; pvT = 0;
  /* —— 连杀提示（comboCount 已在 startLevel 里清，这里清提示态）—— */
  comboT = 0; comboTxt = '';
  comboShown = -99; comboShownN = -1; comboShownTier = -1;
  /* —— 节流计时 —— */
  if (typeof accentT !== 'undefined') accentT = 0;
  if (typeof tipT !== 'undefined') tipT = 0;
  /* —— 教学弱指引：清掉「本局基准」（提示本身一辈子只说一次，不受影响）—— */
  tutSellBase = -1;
  /* —— v9.13：两个「签名缓存」必须作废 ——
     ① sideSig：左侧信息栏的内容签名。它没变就不重绘 DOM，于是新一局会**留着上一局的侧栏内容**。
     ② bgKey / bgCv：战场静态层（网格+路径）缓存。key 含路径指纹，理论上换关会变，
        但同关重开 / 教学关与第 1 关共用 lvIndex 这类情况下会命中缓存 → 贴图与实况不符。
     强制清空最省事，代价只是本局第一帧多画一次静态层。 */
  if (typeof sideSig !== 'undefined') sideSig = '';
  if (typeof bgKey !== 'undefined') bgKey = '';
  if (typeof bgCv !== 'undefined') bgCv = null;
}
...
function startLevel(i, asEndless, noIntro){
  ...
  resetRunState();   /* v9.12：先把所有跨局状态清干净，避免上一局漏进来 */
