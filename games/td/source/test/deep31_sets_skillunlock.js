...
/* v9.15 变更：关卡开幕弹层已整体停用（普通关卡含第 1 关都不再弹），其中的「本关主动技能」段落随之移除；
   技能说明现在 100% 由上面的技能栏「?」面板承担，故本条改为防回归：源码里不应再出现已废的弹层字段。*/
ok(html.indexOf('本关主动技能') < 0, '开幕弹层已停用（v9.15）：源码中不再残留「本关主动技能」段（技能说明归技能栏 ? 面板）');
