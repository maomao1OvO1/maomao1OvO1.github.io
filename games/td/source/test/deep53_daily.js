...
code = code.replace(marker,
  ...
  '  isDailyNow: function(){ return isDaily; }, setDaily: function(v){ isDaily = !!v; }, starcoreNow: function(){ return starcore(); }, gameOverFn: gameOver, levelClearFn: levelClear, el: function(id){ return document.getElementById(id); } };\n' + marker);
var errors = [];
function mkEl(tag){
...
console.log('');
console.log('=== v9.11 星核误发修复（isDaily 状态泄漏）===');
ok(typeof T.setDaily === 'function' && typeof T.isDailyNow === 'function', '测试可读写 isDaily 标记');
ok(T.el('startOv') !== undefined, '环境就绪');

/* 场景复现：模拟「每日挑战失败」—— 把 isDaily 置真，然后走失败结算 */
T.startLevel(0, false, true);
T.setDaily(true);
ok(T.isDailyNow() === true, '已把 isDaily 置为 true（模拟正在打每日挑战）');
var _starBefore = T.starcoreNow();
T.gameOverFn();
ok(T.isDailyNow() === false, '★ 失败结算后 isDaily 已被清掉（修复前会残留为 true → 之后普通关卡误发星核）');

/* 再走一遍普通关卡通关：不应再被当成每日挑战 */
T.startLevel(1, false, true);
ok(T.isDailyNow() === false, '普通关卡开局 isDaily 保持 false');
var _s2 = T.starcoreNow();
T.levelClearFn();
ok(T.starcoreNow() === _s2, '★ 普通关卡通关后星核数量未增加（修复前会 +2~6）');
ok(T.isDailyNow() === false, '通关后 isDaily 同样归零（两条路径都清理）');
